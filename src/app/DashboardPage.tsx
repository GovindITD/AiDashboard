...
import { AIClientQuestionMaster } from "../networkServices/QuestionMaster";
...
export default function DashboardPage() {
  ...
  const getRangeTitle = (baseTitle: string) => {
    ...
    if (fromDate && toDate) {
      ...
      if (
        differenceInDays(toDate, fromDate) === 0 &&
        fromDateStart.getTime() === today.getTime()
      ) {
      ...
      if (
        differenceInDays(toDate, fromDate) === 0 &&
        fromDateStart.getTime() === yesterday.getTime()
      ) {
  ...
  const getRangeDescription = (
    ...
    if (fromDate && toDate) {
      ...
      return `Total ${plural} from ${format(fromDate, "LLL dd")} to ${format(
        toDate,
        "LLL dd"
      )}.`;
    }
  ...
  React.useEffect(() => { 
    const response = AIClientQuestionMaster();
    ...
    <div className="flex min-h-screen w-full flex-col bg-background">
      ...
      <main className="flex flex-1 flex-col gap-8 p-4 md:p-8">
        ...
        <div className="space-y-6">
          ...
          <div>
            <h2 className="text-xl font-bold p-3 rounded-t-lg bg-sky-200 text-sky-800">
              1. Patient Visit in AI
            </h2>
            <div className="grid gap-4 md:grid-cols-2 p-4 border border-t-0 rounded-b-lg border-sky-200">
              <Card
                onClick={handleAiVisitClick}
                className="cursor-pointer bg-sky-50 hover:bg-sky-100 transition-colors"
              >
          ...
          <div>
            <h2 className="text-xl font-bold p-3 rounded-t-lg bg-green-200 text-green-800">
              2. Total Patient Question
            </h2>
